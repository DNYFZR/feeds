...
import DisplayHand, { PlayingCard, PlayingCardSet } from './GamesCards';
...
const TexasHoldEm: React.FC = () => {  
  ...
  const setupDeck = async() => {
    ...
    // Get New Deck 
    if(deckID === null) {
      let call = await fetch("https://deckofcardsapi.com/api/deck/new/shuffle/?deck_count=1", { method: "GET" });
      call = await call.json().then(res => useID = res.deck_id).catch(error => console.error('Error:', error));
    }

    // Shuffle
    await fetch(`https://deckofcardsapi.com/api/deck/${useID}/shuffle/`, { method: "GET" }
  ...
  const getACard = async(n:number) => {
    ...
    let res = await fetch(`https://deckofcardsapi.com/api/deck/${useID}/draw/?count=${n}`, { method: "GET" });
    res = await res.json().then(res => newCards = res.cards);
    return {cards: newCards} as PlayingCardSet;
